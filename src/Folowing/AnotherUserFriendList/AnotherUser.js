        ...
        setFollowers(
          data.docs.map(function (item) {
            if (userInfo.followers.includes(item.data().id)) {
              console.log('yes');
              setFollowerss(function (prev) {
                return [
                  ...prev,
                  { data: item.data(), adress: item._key.path.segments[6] },
                ];
              });
            }
    ...
    function () {
      setFollows(
        followers.map(function (item) {
          ...
          setFollowerss(
            followerss.slice(followerss.length / 2, followerss.length)
          );
        })
      );
      if (get < 2) {
  ...
  return (
    <div className="userblok">
      <List ref={ref} className="listmain" sx={{ marginTop: '10px' }}>
        ...
        >
          <PeopleOutlineIcon
            sx={{ marginLeft: '90px', marginTop: '-180px', color: 'white' }}
          />
        </span>
        <span
        ...
        >
          {get > 1 && mainuser.follows.includes(userInfo.adress) ? (
            <Button
              variant="outlined"
              color="warning"
              size="small"
